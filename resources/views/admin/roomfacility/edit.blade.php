...
<style>
    ...
    .gradient-header {
        background: linear-gradient(135deg, #f59e0b, #d97706, #164e63);
        color: white;
...
<div class="modern-card">
    <div class="gradient-header">
        <h3 class="card-title mb-0" style="font-size: 1.5rem; font-weight: 600;">
            <i class="fas fa-edit mr-2"></i>ROOM FACILITY EDIT
        </h3>
        <div class="edit-badge" style="position: absolute; top: 20px; right: 20px;">
            <i class="fas fa-pencil-alt mr-1"></i>Editing Mode
        </div>
    </div>
    
    <div class="card-body" style="padding: 40px;">
        <form action="{{ route('roomfacility.update', $data->id) }}" method="post" id="facilityEditForm">
            @csrf
            @method('patch')
            ...
            <div class="form-group" style="text-align: right; margin-top: 40px;">
                <button type="submit" class="modern-btn" id="updateBtn">
...
document.addEventListener('DOMContentLoaded', function() {
    ...
    const originalValue = facilityInput.value;
    ...
    // Real-time validation feedback and change detection
    facilityInput.addEventListener('input', function() {
        const value = this.value.trim();
        const hasChanged = value !== originalValue;
        
        if (value.length > 0) {
            this.style.borderColor = hasChanged ? '#f59e0b' : '#10b981';
            this.style.boxShadow = hasChanged ? 
        ...
        if (hasChanged) {
            ...
            updateBtn.style.background = 'linear-gradient(135deg, #f59e0b, #d97706)';
        } else {
            updateBtn.innerHTML = '<i class="fas fa-check"></i> No Changes';
